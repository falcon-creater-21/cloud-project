...
<style>
  ...
  .section-sub { font-size: 0.78rem; color: #9090a0; margin-bottom: 1rem; }
  ...
  .page-sub { color: #9090a0; font-size: 0.9rem; margin-top: 4px; }
...
def generate_data():
    ...
    # FIX: use enumerate() instead of deprecated get_loc()
    sales_data = []
    for month_idx, month in enumerate(months):
        for cat in categories:
            base = base_revenue[cat]
            growth = 1 + (month_idx * 0.015)
            noise = np.random.normal(1, 0.08)
            rev = int(base * growth * noise)
            sales_data.append({
                "month": month,
                "category": cat,
                "revenue": rev,
                "units": int(rev / random.uniform(22, 28)),
                "profit": int(rev * random.uniform(0.18, 0.32)),
            })
    ...
    daily = pd.DataFrame({
        "date": days,
        "transactions": (
            np.random.poisson(340, 90).astype(float)
            + np.sin(np.arange(90) * 0.22) * 40
        ),
    ...
    top_products = pd.DataFrame({
        "product": products,
        "revenue": sorted(np.random.randint(28000, 140000, 10).tolist(), reverse=True),
        "units": sorted(np.random.randint(400, 3200, 10).tolist(), reverse=True),
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
ALL_CATS = ["Electronics", "Apparel", "Home & Garden", "Sports", "Beauty"]
...
with st.sidebar:
    ...
    selected_categories = st.multiselect(
        "Categories", ALL_CATS, default=ALL_CATS
    )
    # Guard: if user deselects everything, fall back to all
    if not selected_categories:
        selected_categories = ALL_CATS

    selected_metric = st.selectbox(
        "Primary Metric", ["Revenue", "Units Sold", "Profit"], index=0
    )

    chart_style = st.selectbox("Chart Style", ["Area", "Bar", "Line"], index=0)

    st.markdown("---")
    st.markdown("**Date Range (months)**")
    ...
    st.markdown("---")
    st.markdown(
        '<p style="font-size:0.7rem;color:#5555aa;text-align:center">'
        "Sales Intelligence v1.1<br>Data: FY 2024</p>",
        unsafe_allow_html=True,
    )
...
metric_col = {"Revenue": "revenue", "Units Sold": "units", "Profit": "profit"}[
    selected_metric
]
...
with col_title:
    st.markdown(
        '<div class="page-title">Sales Intelligence '
        '<span class="accent-pill">LIVE</span></div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="page-sub">Fiscal Year 2024 · All markets · Updated Dec 31</div>',
        unsafe_allow_html=True,
    )
with col_date:
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(
        f'<div style="text-align:right;font-size:0.8rem;color:#9090a0">'
...
with col_trend:
    ...
    st.markdown(
        f'<div class="section-header">{selected_metric} Trend by Category</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="section-sub">Monthly performance across selected categories</div>',
        unsafe_allow_html=True,
    )

    pivot = (
        df_filtered.pivot_table(
            index="month", columns="category", values=metric_col, aggfunc="sum"
        )
        .fillna(0)
    )
        ...
        if chart_style == "Area":
            fig_trend.add_trace(
                go.Scatter(
                    ...
                    fill="tozeroy",
                    fillcolor=color + "20",   # hex alpha — no rgb() manipulation
                )
            )
        elif chart_style == "Bar":
            fig_trend.add_trace(
                go.Bar(x=pivot.index, y=pivot[cat], name=cat, marker_color=color)
            )
        else:
            fig_trend.add_trace(
                go.Scatter(
                    x=pivot.index, y=pivot[cat], name=cat, mode="lines+markers",
    ...
    fig_trend.update_layout(
        ...
        yaxis=dict(
            showgrid=True, gridcolor="#f0eee8", tickfont=dict(size=11),
            tickprefix="" if selected_metric == "Units Sold" else "$",
        ),
        barmode="stack" if chart_style == "Bar" else "overlay",
        hovermode="x unified",
    )
...
with col_donut:
    ...
    st.markdown(
        f'<div class="section-sub">Share of {selected_metric.lower()} by category</div>',
        unsafe_allow_html=True,
    )

    cat_totals = (
        df_filtered.groupby("category")[metric_col].sum().reset_index()
    )
    fig_donut = go.Figure(
        go.Pie(
            labels=cat_totals["category"],
            values=cat_totals[metric_col],
            hole=0.62,
            marker=dict(
    ...
    fig_donut.update_layout(
        ...
        annotations=[
            dict(
                text=f"<b>{len(selected_categories)}</b><br>cats",
...
with col_daily:
    ...
    st.markdown(
        '<div class="section-sub">Last 90 days · orders processed</div>',
        unsafe_allow_html=True,
    )
    fig_daily = go.Figure(
        go.Bar(
...
with col_region:
    ...
    st.markdown(
        '<div class="section-sub">Revenue vs customers · bubble size = satisfaction</div>',
        unsafe_allow_html=True,
    )
    fig_bubble = go.Figure(
        go.Scatter(
            ...
            marker=dict(
                ...
                color=PALETTE[: len(regional)],
                line=dict(color="white", width=2),
                opacity=0.9,
            ),
            hovertemplate=(
                "<b>%{text}</b><br>Customers: %{x:,}<br>Revenue: $%{y:,}<extra></extra>"
            ),
        )
    )
    fig_bubble.update_layout(
        ...
        yaxis=dict(
            title="Revenue ($)", showgrid=True, gridcolor="#f0eee8",
...
st.markdown(
    '<div class="section-sub">Ranked by total revenue · bar colour = relative value</div>',
    unsafe_allow_html=True,
)
...
with col_bar:
    prod8 = top_products.head(8)
    fig_prod = go.Figure(
        go.Bar(
            x=prod8["revenue"], y=prod8["product"],
            orientation="h",
            marker=dict(
                color=prod8["revenue"].tolist(),
                colorscale=[[0, "#e8e6e0"], [1, "#6c63ff"]],
                line=dict(width=0),
            ),
            text=[f"${r:,.0f}" for r in prod8["revenue"]],
    ...
    fig_prod.update_layout(
        height=300, margin=dict(l=0, r=80, t=0, b=0),
        plot_bgcolor="white", paper_bgcolor="white",
...
st.markdown("<hr>", unsafe_allow_html=True)
st.markdown(
    '<div style="text-align:center;font-size:0.75rem;color:#b0b0c0">'
